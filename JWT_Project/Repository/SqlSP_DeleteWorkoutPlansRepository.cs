...
{
    public class SqlSP_DeleteWorkoutPlansRepository : ISP_DeleteWorkoutPlansRepository
    {
        private readonly ApplicationDbContext dbContext;
        public SqlSP_DeleteWorkoutPlansRepository (ApplicationDbContext dbContext)
        {
            this.dbContext = dbContext;
        }
        public async Task<List<ApiResponseMessage>> getasync(SP_DeleteWorkoutPlans input)
        {
            {
                List<ApiResponseMessage> list = new List<ApiResponseMessage>();
                using (var command = dbContext.Database.GetDbConnection().CreateCommand())
                {
                    command.CommandText = "[SP_DeleteWorkoutPlans]";
